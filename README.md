...
## 追加機能: 商品アンケート（カツオ由来エラスチン・サプリメント）

ホームの「このサプリ、買いますか？」バナー、またはメニューの「商品アンケート」から開けます（ルート: `beauty`）。カツオ由来の成分「エラスチン」を使ったサプリメントの企画に向けた、匿名の顧客アンケートです。

- **設問**: `src/main.js` の `beautySurveyQuestions` で管理します。お住まいの地域・性別・年代の回答者属性3問のあと、まず「そもそも興味があるか」（`interest`）と「どんな効果・用途なら欲しいか」（`purpose`）でニーズを先に拾い、そのあとサプリ習慣、価格提示付きの購入意向、買わない場合の代替案、剤形の好み、許容予算、買いたい場所、ためらう理由と続く計13問（単一選択・複数選択・自由記入「その他」欄を質問ごとに使い分け）。末尾に自由記入欄とモニター希望メール欄もあります。イントロ文は `beautySurveyIntro` にあります。
...
- **記録先シート**: 既存の「アンケート」シートとは別に、`apps-script/Code.gs` の `CONFIG.beautySurveySheet`（既定「商品アンケート2」）という専用シートへ、設問ごとに列分けして記録されます（`logBeautySurveyRow`）。設問数が増えたときは見出し行も自動で広がります。既存の旅アンケート集計とは混ざりません。旧設問セットの回答は、シート名を変えたことで「美容液アンケート」「商品アンケート」にそのまま残っています。
