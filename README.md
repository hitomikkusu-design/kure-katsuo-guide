...
## 追加機能: 美容液アンケート（鰹の「チチコ」由来エラスチン）

ホームの「鰹の『チチコ』が、美容液に？」バナー、またはメニューの「美容液アンケート」から開けます（ルート: `beauty`）。久礼のカツオの心臓「チチコ」から採れる成分「エラスチン」を使った美容液の企画に向けた、匿名の顧客アンケートです。QRコード（`https://hitomikkusu-design.github.io/kure-katsuo-guide/#/beauty`）から直接このページへ飛べます。

- **設問**: `src/main.js` の `beautySurveyQuestions` で管理します。お住まいの地域・性別・年代の回答者属性3問に加え、美容液習慣、購入意向、買わない場合の代替案、買いたい用途、テクスチャーの好み、許容予算、買いたい場所、ためらう理由など計11問（単一選択・複数選択・自由記入「その他」欄を質問ごとに使い分け）。末尾に自由記入欄とモニター希望メール欄もあります。イントロ文は `beautySurveyIntro` にあります。
...
- **記録先シート**: 既存の「アンケート」シートとは別に、`apps-script/Code.gs` の `CONFIG.beautySurveySheet`（既定「チチコ美容液アンケート」）という専用シートへ、設問ごとに列分けして記録されます（`logBeautySurveyRow`）。設問数が増えたときは見出し行も自動で広がります。既存の旅アンケート集計とは混ざりません。旧設問（サプリメント版・初期の美容液版）の回答は、シート名を変えたことで「商品アンケート」「美容液アンケート」（Code.gs内の旧シート名）にそのまま残っています。
