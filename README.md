...
## 追加機能: 商品アンケート（カツオの「チチコ」由来エラスチン）

ホームの「鰹の『チチコ』が、商品に？」バナー、またはメニューの「商品アンケート」から開けます（ルート: `beauty`）。久礼のカツオの心臓「チチコ」から採れる成分「エラスチン」を使った商品の企画に向けた、匿名の顧客アンケートです。回答者の約半数が男性で、男性が挙げる関心（血管・関節・疲れにくさ）が女性向けの「肌の悩み」中心の旧設問では拾えていなかったため、男女どちらにも刺さる内容に全面刷新しました。

- **冒頭の説明文**: `beautySurveyIntro.paragraphs` に、エラスチンの機能性表示食品としての受理実績・ヒト試験の出典を含む説明文を段落ごとに保持しています。薬機法・景表法上、要約・言い換えをせず原文のまま表示する運用です（「血管が若返る」等の断定表現は使用していません）。
- **設問**: `src/main.js` の `beautySurveyQuestions` で管理します。年代・性別のあと、体のしなやかさで気になること、エラスチンの認知度、説明を読んで信頼できると感じた点、関心の変化、機能性表示商品への購入意向、健康のために続けていること、続けやすい形、月額・お試しサイズの許容価格、買いたい場所、贈答用か自分用か、未利用部位（チチコ）を使うことへの気持ち、自由記入の計15問です。
...
- **記録先シート（性別で完全分離）**: `apps-script/Code.gs` の `CONFIG.maleSurveySheet`（既定「男性調査」）・`CONFIG.femaleSurveySheet`（既定「女性調査」）へ、Q2「性別」の回答が「男性」かどうかで書き込み先を振り分けます（`logBeautySurveyRow`）。各シートは設問ごとに列分け＋末尾に `submissionId` 列を持ち、書き込み前にシートの見出し行と送信内容のキーが一致するか検証、**一致しない場合は書き込まずエラーを返します**（自動で見出しを広げる旧仕様は廃止）。旧シート（「商品アンケート」「商品アンケート2」「商品アンケート3」「美容液アンケート」）は初回アクセス時に自動で「〜（重複あり・旧データ）」へリネームされ、上書き・削除はされません（`renameOldBeautySheetOnce`）。
- **二重送信対策**: フォーム表示のたびにクライアント側で `submissionId`（UUID）を1つ発行し、送信ペイロードに含めます。送信ボタンは押した瞬間に無効化され（連打対策）、通信の成否にかかわらず必ず完了画面へ遷移します。サーバー側では同一 `submissionId` の再送信を書き込まずに成功扱いで返し（冪等化）、`submissionId` が変わってしまうケース（ブラウザの戻る操作後の再読み込みなど）への保険として「直前の1行と内容が完全一致し、かつ60秒以内」の書き込みも弾きます（`isDuplicateSubmissionId` / `isDuplicateRecentContent`）。
- **モニター・先行案内の導線**: 回答送信の完了画面のあと、単独ステップとして「発売前のモニター・先行案内をお送りしてもよろしいですか。」を表示し、お名前・メールアドレス（いずれも任意）を `formType:'beautyMonitorSignup'` として `CONFIG.beautyMonitorSheet`（既定「モニター登録」）シートへ記録します（`logBeautyMonitorRow`）。「今回はやめておく」でスキップも可能です（控えめなテキストリンクとして配置）。以前はアンケート末尾の任意項目に埋もれており、78件中2件しか連絡先を取得できていませんでした。
