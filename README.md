...
## 追加機能: 商品アンケート（カツオの「チチコ」由来エラスチン・サプリ／美容液）

ホームの「このサプリ、買いますか？」バナー、またはメニューの「商品アンケート」から開けます（ルート: `beauty`）。久礼のカツオの心臓「チチコ」から採れる成分「エラスチン」を使った「飲むサプリメント」「塗る美容液」の企画に向けた、匿名の顧客アンケートです。

- **設問**: `src/main.js` の `beautySurveyQuestions` で管理します。お住まいの地域・性別・年代の回答者属性3問のあと、興味度（`interest`）、贈答用か自分用か（`giftPurpose`）、現在使用中のブランド・価格帯（`currentBrand`・自由記入・任意）、チチコ由来と知った時の興味変化（`chichikoAwareness`）、欲しい効果・用途（`purpose`）、サプリ習慣、価格提示付きの購入意向（`purchaseIntent`）、お試しサイズの許容価格（`trialSizeBudget`）、買わない場合の代替案、剤形の好み、許容予算、買いたい場所（「久礼大正町市場の店頭」を含む）、ためらう理由と続きます。イントロ文は `beautySurveyIntro` にあります。
- **未成年の分岐**: 年代で「10代以下」を選ぶと、年代より後の設問（`#beauty-survey-post-age`）が自動的に非表示になり、案内文を出してそのまま送信できます（`updateBeautySurveyMinorBranch`）。観光客の子どもの回答が集計を歪める問題への対処です。
...
- **記録先シート**: 既存の「アンケート」シートとは別に、`apps-script/Code.gs` の `CONFIG.beautySurveySheet`（既定「商品アンケート3」）という専用シートへ、設問ごとに列分け＋末尾に `submissionId` 列を付けて記録されます（`logBeautySurveyRow`）。書き込み前にシートの見出し行と送信内容のキーが一致するか検証し、**一致しない場合は書き込まずエラーを返します**（自動で見出しを広げる旧仕様は廃止。列ズレの再発を構造的に防ぐため）。旧シート「商品アンケート2」（列ズレ・二重送信の混入データ）は初回アクセス時に自動で「商品アンケート2（重複あり・旧データ）」へリネームされ、上書き・削除はされません。
...
- **モニター・先行案内の導線**: 回答送信の完了画面のあと、単独ステップとして「発売前のモニター・先行案内を受け取りますか？」を表示し、メールアドレスを `formType:'beautyMonitorSignup'` として `CONFIG.beautyMonitorSheet`（既定「モニター登録」）シートへ記録します（`logBeautyMonitorRow`）。「今回はやめておく」でスキップも可能です。以前はアンケート末尾の任意項目に埋もれており、78件中2件しか取得できていませんでした。
